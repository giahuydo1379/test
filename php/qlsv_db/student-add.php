...
if (!empty($_POST['add_student'])) {
    ...
    if (empty($data['sv_name'])) {
        $errors['sv_nmae'] = 'Chưa nhập tên sinh viên';
    }
    if (empty($data['sv_sex'])) {
        $errors['sv_sex'] = 'Chưa nhập giới tinh sinh viên';
    }
    //Nếu không có lỗi thì insert
    if (!$errors) {
        add_student($data['sv_name'], $data['sv__sex'], $data['sv_birthday']);
            ...
            <table width="50%" border="1" cellspacing="0" cellpadding="10">
                <tr>
                    ...
                    <td>
                        ...
                        <?php if (!empty($errors['sv_name'])) echo $erros['sv_name']; ?>
                    </td>
                </tr>
                <tr>
                    <td>Gender</td>
